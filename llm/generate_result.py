...
PRINT_REQUEST_PROMPT = True # 프롬프트 출력 여부
...
def create_consulting_result(industry, company_size, pain_point):
    result_contents = []
    result_contents.append(get_result_content(title=CONSULTING_RESULT_TITLES[0], content=get_industry_example_content(industry=industry, company_size=company_size))) # AI 사례

    recommend_ai_answer, ai_service = get_ai_service_datas(pain_point=pain_point, industry=industry, company_size=company_size)
    ...
    result_contents.append(get_result_content(title=CONSULTING_RESULT_TITLES[2], content=get_ai_service_require_data(industry, ai_service, data_category_rag))) # AI 도입 시 필요 데이터

    result_contents.append(get_result_content(title=CONSULTING_RESULT_TITLES[3],
                       content=get_ai_service_process(ai_service=ai_service, data_category_rag=data_category_rag, industry=industry, company_size=company_size))) # AI 도입 프로세스

    result_contents.append(get_result_content(title=CONSULTING_RESULT_TITLES[4],
                       content=get_ai_service_ROI(ai_service=ai_service, industry=industry, company_size=company_size, recommend_ai_answer=recommend_ai_answer))) # 예상 비용 및 ROI
...
def get_industry_example_content(industry, company_size):
    ...
    return get_LLM_answer_with_validate(prompt=prompt, rag_data=rag_data)
...
def get_ai_service_process(ai_service, data_category_rag, industry, company_size):
    ...
    return get_LLM_answer_with_validate(prompt, rag_data + data_category_rag)
...
def get_ai_service_ROI(ai_service, industry, company_size, recommend_ai_answer):
    ...
    return get_LLM_answer_with_validate(prompt, rag_data + ai_service)


# AI 컨설팅 데이터 요약
def get_summary_result(result):
    prompt = generate_summary_result_prompt(result)
    return get_LLM_answer_with_validate(prompt=prompt, rag_data=result)
...
def get_LLM_answer_with_validate(prompt, rag_data, temperature=0.7):
    while(True):
        answer = get_chat_gpt_answer(prompt=prompt, temperature=temperature)

        if(validate_hallucination(rag_data=rag_data, answer=answer) is True):
            return answer
